...
function FilterWidget({ onFiltersChange, filters, onReset, onClose, filterOptions }) {
    ...
    return (
        <div className="w-full sm:w-11/12 mx-auto p-1 pb-2 md:pb-4 bg-pure_white md:text-xs bg-blur outline outline-pure_white/20 shadow-sm md:shadow-none rounded-md caret-transparent">
            <div className="flex justify-end h-fit mr-4 md:mt-4 md:hidden">
            ...
            <div className="grid grid-cols-1 divide-y-2 gap-1 p-3 md:p-1 divide-light_dark/10">
            ...
            <div className="flex flex-row space-x-2 w-fit mt-2.5 mx-auto items-center justify-center p-1 sm:w-full sm:justify-even">
                <button onClick={onReset} className="rounded-md w-40 h-8 sm:h-12 md:h-6 sm:w-64 drop-shadow-xl md:p-1 cursor-pointer md:text-ssm outline outline-light_dark/15 text-default/75 font-semibold active:bg-gray/35">Clear all filters</button>
                <button onClick={handleApplyClick} disabled={!hasChanges} className="rounded-md md:hidden cursor-pointer w-40 p-1 h-8 sm:h-12 sm:w-64 md:h-fit md:w- md:p-3 bg-bg_variant1 text-pure_white/75 font-bold">APPLY</button>
            </div>
        </div>
    );
}
