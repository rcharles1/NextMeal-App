...
function MealCard({ meal }) {
    return (
        <>
            {meal ? (
                <div className="flex flex-col space-y-4 h-64 w-[11rem] rounded-xl sm:rounded-2xl caret-transparent px-2.5 pb-3.5 pt-3 text-base bg-pure_white drop-shadow-sm text-start text-default/75 sm:h-72 sm:w-56">
                    ...
                    <div className="w-fit h-fit ml-16 border-b-2 justify-center flex flex-row space-x-1">
                        <span><NavLink to={`/mealitem/${meal._id}`} className="font-semibold text-sm sm:text-base active:text-bg_variant1">Explore</NavLink></span>
